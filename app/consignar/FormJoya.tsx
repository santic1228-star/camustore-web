...
import { ANILLO_NOMBRES, PENDIENTE_NOMBRES, esJoyaBarata, type TipoJoya } from "@/lib/precios";
...
/** Espejo de SeccionJoyeria del cotizador. */
export default function FormJoya({ onAgregar }: Props) {
  ...
  if (!tipo) motivo = "Elegí si es anillo o pendiente.";
  else if (!nombre) motivo = "Elegí el nombre de la joya.";
  else if (!tieneLife) motivo = "Solo se consignan joyas con la opción Life Recovery.";
  else if (tipo === "anillo") {
    if (!hpDdRef) motivo = "Los anillos se consignan solo con HP + DD + REF.";
  } else {
    if (!exeRate || !dmg2pct) motivo = "Los pendientes necesitan exe rate 10% + dmg 2% (obligatorias).";
  }
        ...
        {tipo && (
          <>
            ...
            <div>
              ...
              {!tieneLife && (
                <p className="text-[10px] font-body text-neon-orange/80 mt-1.5">
                  Solo se consignan joyas con Life Recovery.
                </p>
              )}
            </div>
